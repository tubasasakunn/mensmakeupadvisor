...
このアプリの TikTok / Lemon8 カルーセル投稿は、**ルートの自動運用ワークスペース**（`~/android`）が回す。
このリポジトリ側に必要なのは **`material/` だけ**（SNSエンジンのコードは持たない＝ルートに集約済み）。

- `material/manifest.json` … アプリの素性＋ブランド（色 / ワードマーク / アイコン / motif / bundleId / ascAppId / concept / タグ）。
- `material/OVERVIEW.md` … コンテンツ正本（コンセプト・コンテンツ知識・過去の投稿例）。
- `material/app_icon_1024.png` … ワードマーク用アイコン（**必須**）。`material/symbols/*.svg` … アプリ固有の装飾シンボル。
- `material/`（スクショ・`footage/`）… 実アプリ画面・背景素材。

**画像の作り方・デザインの正本**はルートの `carousel-craft` スキル（`~/.claude/skills/carousel-craft/`）。
**App Store 提出用のストア画像**（`scripts/make_store_images.py`・`release/<version>/`）は SNS とは別物で、従来どおりこのリポジトリの関心。
